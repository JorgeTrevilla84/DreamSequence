...
if (obj_player.keyZoom) {
	...
	if (global.soundZoomInReset){
		audio_play_sound(snd_zoom_out, 10, false);
		global.soundZoomInReset = false;
		global.soundZoomOutReset = true;
	}
	
	global.playerLocked = true;
} else {
	zoom_level = 1;
	if (global.soundZoomOutReset){
		audio_play_sound(snd_obvious, 10, false);
